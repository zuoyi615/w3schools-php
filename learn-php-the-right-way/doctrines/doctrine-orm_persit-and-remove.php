...
try {
    ...
    foreach ($items as [$description, $quantity, $unitPrice]) {
        ...
        // $entityManager->persist($item); // if without cascade:['persist', 'remove'], this is required
    }

    $entityManager->persist($invoice);
    $entityManager->flush(); // without flush(), would not write to database
    // $entityManager->getUnitOfWork()->size(); // 4

    // $entityManager->remove($invoice); // after persist
    // $entityManager->flush();
