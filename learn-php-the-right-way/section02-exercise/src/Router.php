<?php
  ...
  use Exercise02\Exceptions\RouteNotFoundException;

  class Router {
    private array $routes = [];

    private function register(string $route, string $method, callable|array $action): void {
    ...
    public function resolve(string $uri, string $method): mixed {
      $route  = explode('?', $uri)[0];
      $action = $this->routes[$route][strtolower($method)] ?? null;
